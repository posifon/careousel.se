	...
	if ( 'edit-tags.php' == WPGlobusVendor.pagenow || 'term.php' == WPGlobusVendor.pagenow ) {
		...
		api = WPGlobusYoastSeo = {
			...
			updateWpseoKeyword: function( kw, l ) {
				/** update focus keyword in init time */
				$( '#wpseo-meta-section-content_' + l + ' .wpseo_keyword'  ).text( kw );
			},	
			start: function() {
				...
				/** @todo remove after test, obsolete since yoastseo 3.2 */
				//api.qtip();	
		...
		WPGlobusYoastSeoPlugin.prototype.setScoreIcon = function() {
			...
			var timer = function() {
				...
				var classes = $('#wpseo-metabox-tabs .wpseo-score-icon').attr('class');
				classes = classes.replace( 'wpseo-score-icon ', '');
				if ( classes == 'bad' || classes == 'ok' || classes == 'good' ) {
					$( '#wpseo-metabox-tabs_'+l+' .wpseo-score-icon' ).removeClass( 'bad ok good 100' );
					$( '#wpseo-metabox-tabs_'+l+' .wpseo-score-icon' ).addClass( classes );
					clearInterval(iID);
				}	
			}	
			iID = setInterval( timer, 700 );		
		}
		...
		// @todo remove after test obsolete from yoastseo 3.2
